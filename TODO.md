...
**Die Sachfrage ist entschieden, und zwar ohne Spieltest.** Der Auftraggeber hat sie gestellt: Ein Schild läuft ab — ist das Ziel danach geheilt? Nein. Eine Barriere verhindert Schaden, sie stellt keine Gesundheit her.

**Und die Heilung stört die Barriere nicht.** Der Einwand, eine Heilung könnte den Verbrauch von The Blackest Night verhindern und damit Dark Arts kosten, trägt nicht: Eine Barriere absorbiert eingehenden Schaden, bevor er die HP erreicht, und wie schnell sie aufgezehrt wird, hängt allein am eingehenden Schaden — der Gesundheitsstand des Trägers ändert daran nichts (Konzept 09). Die einzige Kopplung läuft in die andere Richtung: Stirbt der Träger vor dem Aufzehren, entfällt Dark Arts. Heilung wirkt für den Auslöser, nicht gegen ihn. Es gibt damit auch von dieser Seite keinen Grund, wegen einer Barriere zu warten — wohl aber einen gegen Schadensminderung, und der steht im Eintrag zur Zeitpunktwahl. Beide Ausgänge lassen den Träger genau dort stehen, wo er vorher stand: Läuft sie ungenutzt ab, sind die HP unverändert; fängt sie den Treffer, ist sie verbraucht und die HP sind unverändert. Verändert hat sich nur die verbleibende Zeit — der nächste Treffer kommt jetzt auf einen Träger ohne Barriere, und der Heiler beginnt später als er müsste. Die Anrechnung verrechnet also eine Größe, die Schaden abwehrt, mit einer Größe, die Schaden ausgleicht, und behandelt die Barriere wie bereits geflossene Heilung.
...
**Die Entstehung erklärt den Fehler und verbindet ihn mit dem Eintrag darüber.** Der einführende Commit `27c7b6942` (13.08.2026) heißt „Weigh shield magnitude and duration in heal-**priority** decisions" und begründet so: „a full-HP-plus-shielded ally can look no less urgent **than an unshielded one** at the same raw HP". Das ist ein Vergleich **zwischen Zielen** — genau die Frage, die der Auftraggeber als die einzige benannt hat, bei der ein Schild zählen darf. Angefasst wurden dann `StatusHelper` und `StateUpdater`; die Zielwahl in `ActionTargetInfo` blieb unberührt. Die Absicht war die Priorisierung, die Umsetzung wurde eine Schwelle — Ignorant Surgery im Sinne von Parnas: die Änderung wurde ohne Verständnis der Stelle vorgenommen, an der die gemeinte Entscheidung fällt.

Die Selbstrechtfertigung derselben Nachricht enthält den Fehler bereits: „This only ever raises the computed health value, so it can only defer a heal, never suppress a genuine emergency." Das Verzögern **ist** der Schaden; die Entwarnung setzt voraus, was zu prüfen war.

**Es ist kein Defektklassenfall — erhoben, nicht angenommen.** `GetEffectiveHpPercent` hat 18 Leser im Baum. Alle übrigen stellen die **Überlebensfrage**, und dafür ist die Größe richtig: der Notfall-Provoke auf einen sterbenden Co-Tank (`ObjectHelper.cs:126`), die Starfall-Abwägung in `PhantomDefault`, die Blaumagier-Schwellen, die Immunitätsgrenze des Jagd-Dolls. Nur die beiden Stellen im `StateUpdater` stellen die Heilbedarfsfrage — und nur dort ist die Größe falsch.

**Das entscheidende Argument ist die Additivität, und es stammt vom Auftraggeber:** Ein vollgeheilter Tank **mit** Schild ist besser geschützt als ein geschildeter Tank mit wenig Gesundheit. Schild und Gesundheit ersetzen einander nicht, sie addieren sich. Die Anrechnung behandelt sie als austauschbar — `Math.Max(h, (HP + Schild) / MaxHP)` — und benutzt den Schild als Grund, die Gesundheit **nicht** aufzufüllen. Eine laufende Barriere ist aber die Gelegenheit, das Polster zu vergrößern, nicht der Ersatz dafür.

**Das schließt mehr aus als die Zeitbetrachtung.** Mein bisheriges Argument lautete: Die Barriere läuft ab oder wird verbraucht, und danach steht der Träger dort, wo er stand. Das trifft zu, lässt aber die Lesart offen, die Anrechnung sei wenigstens *während* der Barriere richtig. Ist sie nicht: In jedem Augenblick ihres Bestehens überlebt `Gesundheit + Schild` mehr als `Schild allein`. Damit fällt auch die Fassung, die nur die Notfallfrage anrechnen würde — selbst für „überlebt er den nächsten Treffer" hält die Anrechnung genau die Heilung zurück, die das Überleben sicherer macht.

**Zwei Fragen an derselben Barriere, die nicht zusammengehören.** Die eine: *Was verhindert, dass die Barriere aufgezehrt wird?* Antwort: Schadensminderung — Heilung nicht, und ein zweiter Schild nicht, solange er nachrangig abgebaut wird. Das ist die Regel im Eintrag zur Zeitpunktwahl, und sie ist gebaut. Die andere, hier: *Ist eine laufende Barriere ein Grund, später zu heilen?* Das ist die Anrechnung, und sie hat mit dem Aufzehren nichts zu tun — sie behandelt die Barriere wie bereits geflossene Heilung.

**Der Fall, in dem Heilung tatsächlich schadet, ist ein anderer und bereits geregelt:** Unter Walking Dead heilt sich der Träger durch seine eigenen Aktionen (Cure Potency 1500 je Waffenfertigkeit oder Zauber), und eine Fremdheilung über null nimmt den Todeseffekt weg — nicht ein verschwendeter Zauber, sondern der Verlust des Auslösers. Dafür steht `IsHeldForDeathTrigger` zehn Zeilen unter der Anrechnung, mit eigener Begründung. The Blackest Night hat keine Selbstheilung; dort gibt es diesen Grund nicht.

**Für die Anrechnung ist die Verbrauchsreihenfolge ohne Belang**, anders als für die Sperre: Sie liest `ICharacter.ShieldPercentage`, und das ist die **Summe aller** Barrieren. Ob The Blackest Night vor Divine Benison abgebaut wird, sieht sie nicht — sie rechnet beide zusammen auf die Quote. Daher die Rückkopplung beim Weißmagier: Der eigene Schild hebt die eigene Heilschwelle.

**Entscheidungsgrundlage, am Artefakt erhoben.** Zwei Stellen, beide Einzelziel: `StateUpdater.ShouldHealSelf` und `ShouldHealSingle`. Die Flächenheilung ist nicht betroffen. Der Mechanismus ist `h = Math.Max(h, GetEffectiveHpPercent() / 100f)`, also `(HP + Schild) / MaxHP` — die gemeldete Quote steigt um den Schildanteil. Gemessen wird sie gegen `HealthSingleAbility` 0,70 und `HealthSingleSpell` 0,65 (bzw. 0,65 / 0,55 mit laufendem HoT). `ShieldStatus` führt 61 Barrieren-Ids; die größte benannte ist The Blackest Night mit 25 % der maximalen HP (`ActionId.resx` 7393), die Heilerbarrieren sind in Potenz ausgedrückt und laufzeitabhängig. Fork-Arbeit seit `27c7b6942`; Upstream kennt die Anrechnung nicht.

**Spielwirkung in Zahlen:** Trägt der Tank The Blackest Night, springt die oGCD-Heilung erst bei real **45 %** an statt bei 70 %, die Zauber-Heilung erst bei **40 %** statt 65 %.

| | Option | Folge im Kampf | Betroffene |
|---|---|---|---|
| **N** | Nullvariante: belassen, Voreinstellung aus | Für den Auftraggeber ändert sich nichts. Im Menü bleibt ein Schalter, dessen eine Stellung belegbar später heilt — wer ihn einschaltet, bekommt die 25 Prozentpunkte | N |
| **A** | Anrechnung und Schalter entfernen | Verhalten endgültig wie Upstream. Für jeden, der den Schalter an hatte, kommt die Heilung wieder bei den normalen Schwellen. Das Feld bleibt als ungelesener Eintrag in gespeicherten Konfigurationen zurück — folgenlos, weil `bool` und kein Ordinal-Vertrag | N, U (eine Abweichung weniger) |
| B | Auf Schwellenabsenkung umbauen, wie bei der Unverwundbarkeit (`Math.Min(normal, eigener Wert)`) | Heilung käme später, aber nie gar nicht. **Fällt mit der Additivität**: Auch eine mildere Verzögerung hält Heilung zurück, die das Polster vergrößern würde | N, R |
| C | Nur für die Notfallfrage anrechnen („überlebt er den nächsten Treffer") | **Fällt ebenfalls**: Gerade dort ist `Gesundheit + Schild` die überlebensfähigere Lage, nicht `Schild allein`. Die Anrechnung hielte die Heilung zurück, die den Treffer überstehen hilft | N, R |

**Empfehlung: A — die Anrechnung samt Schalter entfernen**, Verhalten also endgültig auf Upstream. Von den vier Optionen tragen nach dem Additivitätsargument nur noch die Nullvariante und das Entfernen; B und C sind widerlegt, nicht bloß nachrangig. Sie kann im Spiel nichts gewinnen, was ihren Preis wert wäre: Im besten Fall spart sie eine Heilung, die ohnehin nur Ressourcen gekostet hätte, im schlechtesten schaltet sie die oGCD-Heilung im Moment der größten Not ab (der „zu spät gezündet"-Fall oben). Das ist die Richtung, die der Spielweise des Auftraggebers — Sicherheit der Gruppe vor Schadensausstoß — entgegensteht. Ein Schalter, dessen eine Stellung belegbar schlechter ist, ist keine Wahlmöglichkeit, sondern eine Falle. **Vor der Entscheidung des Auftraggebers wird nichts entfernt.**
