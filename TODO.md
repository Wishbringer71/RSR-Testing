...
**Die Sperre ist einseitig gebaut, und die fehlende Seite ist die häufigere.** `ShouldUseBlackestNightOnSelf` fragt **vor** der Zündung, ob der Schadensstrom bereits gedrosselt ist: große Minderung, laufende Gruppenbetäubung, verlangsamtes Paket, Reflexion gewirkt, Rückstoß gewirkt. Steht die Barriere dagegen schon, hindert nichts die Drosselung danach. Im Kampf heißt das: Der Dunkelritter zündet The Blackest Night in den Pull, wirkt eine Sekunde später Reflexion oder Rückstoß — und drückt den Schadensstrom unter die Rate, die seine eigene Barriere in sieben Sekunden aufzehren würde. Die Barriere läuft ungebrochen ab, Dark Arts bleibt aus, die 3000 MP sind ausgegeben und der Rabatt auf den nächsten Edge of Shadow kommt nicht.

**Was dabei fehlt, ist nicht die Minderung, sondern ihre Reihenfolge.** Gegen einen Tankbuster ist Reflexion nach der Barriere richtig — dort zählt der abgewehrte Schaden mehr als die Ressource, und der Tankbuster-Zweig kennt die Staffelungsbedingung deshalb zu Recht nicht. Im Pull ist es umgekehrt, und genau diese Unterscheidung trifft der Code bereits in der Zündrichtung; in der Gegenrichtung fehlt sie. Dasselbe gilt für `ShouldUseArmsLengthOnPull`: Die Regel prüft `!PackSlowed()`, also ob schon verlangsamt ist — nicht, ob gerade eine eigene Barriere aufgezehrt werden soll.

**Die Verlangsamung ist überhaupt nicht abgedeckt.** Die vorhandene Heilerbremse (`HoldHolyForBlackestNight`) hält allein die **Betäubung** von Sanctus zurück. Ein verlangsamtes Paket drosselt den Strom für die Dauer des Debuffs, und `StatusHelper.SlowStatus` hat im ganzen Baum genau einen Leser — `PackSlowed` in der Dunkelritter-Rotation. Auf der Barrierenseite fragt niemand danach.

**Auflösungsbedingung:** eine Beobachtung, wie oft im Pull eine Minderung nach der Barriere fällt. Der Baustein dafür liegt bereit: `StatusHelper.FullAbsorbRewardStatus` ist genau für diese Frage angelegt („rules that would otherwise stop the damage stream while such a barrier is up") und hat bisher zwei Leser, beide in der Sanctus-Bremse. **Empfehlung: nach der Entscheidung über die Schildanrechnung aufgreifen** — beides sind Regeln über dieselbe Barriere, und sie sollten nicht in zwei Anläufen gebaut werden.
