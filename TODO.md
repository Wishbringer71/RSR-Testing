...
## `StartOnFieldOpInCombat2`: beide Restbefunde zurückgezogen

Die nachgeholte Falsifikationsstufe widerlegt beide. **Der Optionstext ist der Absichtsbeleg:** „Auto turn on auto mode when in combat in Bozja/Eureka/Occult Fate/CE" — die Nachbaroptionen nennen ihr Subjekt ausdrücklich („when **party** is in combat", „when **alliance** is in combat", „when **attacked**"), diese nicht. Der Zweig meint also den eigenen Kampfeintritt; dass der Spieler in der Liste steht und auslöst, ist Absicht, keine Lücke, und es kollidiert auch nicht mit `StartOnAttackedBySomeone2`, das den anderen Fall (angegriffen werden) abdeckt.

Damit fällt auch der zweite Befund: `AllHostileTargets.Contains(t)` als Übersprungkriterium ist unter dieser Lesart stimmig — Gegner, die RSR ohnehin als Ziele führt, brauchen keinen Sonderweg zum Autostart, Objekte außerhalb dieser Liste dagegen schon. Ein Wechsel auf `t.IsEnemy()` wäre eine Verhaltensänderung ohne Beleg, keine Korrektur.

**Fehlerfortpflanzung offengelegt:** Die Begründung des bereits umgesetzten Puppen-Fixes (`33f8cdff`) war zu eng — der Zweig reagiert nicht primär auf Mitspieler, sondern auf den eigenen Kampf, die Schleife über andere Objekte ist die Ergänzung. Der Fix selbst bleibt richtig: eine Übungspuppe im Kampf soll den Autostart nicht auslösen, wenn der Spieler nicht kämpft.
