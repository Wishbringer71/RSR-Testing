...
Die Zuordnung PvE gegen PvP leistet der Statustext nicht: Beide Formen tragen denselben Anzeigenamen und dieselbe Wirkbeschreibung. `scan13.py` entscheidet sie deshalb an der **Aktion** gleichen Namens (A41).

- **15 Ids stehen hinter einer PvE-Aktion, die eine Barriere erzeugt.** Zehn davon sind Jobbarrieren: `ShakeItOff` (1457) und `ShakeItOff_1993` (1993, WAR), `SeraphicVeil` (1917), `SeraphicVeil_2040` (2040) und `SeraphicVeil_3097` (3097, SCH-Seraph), `NeutralSect_1921` (1921) und `NeutralSect_3988` (3988, AST), `TheSpire_3892` (3892, AST-Karte), `ImprovisedFinish` (2697, DNC), `DivineCaress` (3903, WHM). Fünf stammen aus dem Occult Crescent: `OccultUnicorn` (4243), `BlessedRain` (4253), `MagicShell` (4788), `SteadfastStance` (4800), `Lance` (5319).
- **Die übrigen 46 gehören nicht hinein:** PvP-Formen ganzer Ketten (GNB `AbdomenTear`/`JugularRip`/`EyeGouge`/`Hypervelocity`/`FatedBrand`, RDM `EnchantedRiposte`/`EnchantedZwerchhau`/`EnchantedRedoublement`/`Corpsacorps`/`Engagement`/`Forte`, dazu `Huton_3186`, `OgiNamikiri`, `KaeshiNamikiri`, `Thunderclap_3173`, `ArmoredScales` aus Snake Scales PvP, `NocturnalBenefic` aus Aspected Benefic PvP), die entfernten Nocturnal-Sekt-Status des Astrologen sowie Duty- und Gegenstandseffekte ohne Spieleraktion (`AlexandrianBarrier`, `BlessingOfEarth`, `BlessingOfTheVoid`, `DamageBarrier`, `HeartOfTheRroneek`, `StoneWall`, `TrueBenison`, `SomanouticBarrier`, `NeedleVeil`, `Manawall`, `SnowFort`).
- **Korrektur der früheren Aufzählung** (C28): Hier standen sechs Ids als PvE-Kandidaten. Vier davon sind es nicht — `Aquaveil_3086` und `HolySheltron_3026` sind die PvP-Formen, denn in PvE senken beide Aktionen nur den erlittenen Schaden (Aquaveil 15 %, Holy Sheltron 15 % plus Knight's Resolve); `Epicycle` hat nur eine PvP-Aktion, `GuardiansWill` gar keine Aktion dieses Namens. Umgekehrt fehlten acht der zehn Jobbarrieren, darunter mit Shake It Off und Seraphic Veil zwei, die ein Heiler in nahezu jedem Gruppenkampf sieht.

**Empfehlung: die zehn Jobbarrieren aufnehmen, die fünf Occult-Ids zurückstellen.** Die zehn sind belegte Defektstellen derselben Klasse, die A37 bereits behoben hat — eine fehlende Id kehrt die Antwort um, statt sie zu vergröbern —, und sie liegen im Nutzungsprofil. Die Occult-Ids gehören zu einem Sonderinhalt; ihre Aufnahme ist harmlos, aber ohne Nutzen, solange der Auftraggeber ihn nicht spielt. Die Umsetzung ist nicht freigegeben.
