...
**Der schwerste Einzelfall ist ein Kurzschluss, nicht ein Maß:** `tankTars[0]` unter `HealthTankRatio` beendet die Suche sofort, also wird ein Schadensausteiler bei 10 % übergangen, sobald der Tank bei 44 % steht — der von der Vorgabe ausdrücklich genannte Fall, und er ist heute falsch entschieden.

**Empfehlung: in drei Stufen, nach Schweregrad geordnet.** Stufe 1 stellt eine Notrangstufe vor beide Rollen-Kurzschlüsse: Wer unter `HealthForDyingTanks` steht, kommt zuerst. Eine Bedingung, kein neuer Messbaustein, und sie behebt den Fall, in dem jemand stirbt. Stufe 2 lässt Aggro und angekündigten Flächenschaden die Rollenabkürzungen tragen und macht das Maß in Rang 4 von der Lage abhängig — absolute Punkte bei angekündigtem Flächenschaden, sonst Prozentsatz; die beiden Maße sind Surrogate mit gegenläufigen Bruchstellen, ein Tausch allein verschiebt den Fehler nur. Stufe 3 ergänzt die Rate für den Fall mehrerer Betroffener und setzt den Aufnehmer voraus.

**Stufe 1 ist eine belegte Defektbehebung** und gehört nicht hinter eine Option. Stufe 2 und 3 sind Verbesserungen, deren Nutzen ohne Spielbeobachtung eine Annahme bleibt — nach Projektregel hinter eine Einstellung mit beibehaltener Voreinstellung.

**Stufe 1 löst den Schwellenbefund an seiner Ursache mit auf:** Sobald die Aggro gemessen statt über die Rolle unterstellt wird, braucht es keine gegenläufigen Zahlen mehr — wer die Aggro hält, steht vorn, sonst greift die Rangfolge Heiler vor Tank. **Die Schwellen bis dahin nicht anzufassen ist Teil der Empfehlung:** `HealthHealerRatio` allein auf Tankhöhe zu ziehen tauscht den Heiler-mit-Aggro-Fall gegen den Regelfall ein und wiche ohne gemessenen Nutzen vom Upstream ab. Es sind reine Zielwahlschwellen — `healRatio` filtert das Kandidatenfeld vorher, ob überhaupt geheilt wird, entscheiden `HealthSingleAbility`/`HealthSingleSpell` —, ein Heben erzeugt also keine Überheilung, sondern verschiebt nur die Reihenfolge.
