...
**Umgesetzt:** `WHM_Reborn.ShouldHoldHolyWhilePackSlowed`, hinter `HoldHolyWhilePackSlowed` mit Standard **an**. Die Bedingung ist die **Mehrzahl der Gegner im Wirkradius von Sanctus** — strikt mehr als die Hälfte, ohne Mindestzahl; die erste Fassung hatte die Anteilsregel von `PackSlowed` übernommen und wurde vom Auftraggeber unmittelbar korrigiert (C50). Die beiden Regeln beantworten verschiedene Fragen: `PackSlowed` misst über die Jobreichweite, ob der Strom auf den Tank dünn genug ist, um eine Barriere stranden zu lassen, und braucht dort eine Untergrenze; hier hat der Wirkradius die Menge bereits auf das eingegrenzt, was dieser Zauber trifft. Die Ersatzgarantie des Betäubungszweigs gilt unverändert: Ohne einen DoT mit eigenem Wert wird Sanctus gewirkt, gleich wie stark das Paket verlangsamt ist — deshalb werden aus fünfzehn Sekunden Verlangsamung keine fünfzehn Sekunden ohne Sanctus.
