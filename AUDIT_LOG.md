...
**Umgesetzt:** `WHM_Reborn.ShouldHoldHolyWhilePackSlowed`, hinter `HoldHolyWhilePackSlowed` mit Standard **an**. Maßgeblich ist die Zahl der **nicht** verlangsamten Gegner im Wirkradius, gemessen an `Config.AoeCount` der Aktion selbst — derselben Zahl, die `ActionTargetInfo` für die Flächenentscheidung benutzt, also genau der Mindestzahl, die ohne jede Verlangsamung gilt. Ein verlangsamter Gegner ist bereits versorgt und zählt nicht für den Fall, betäuben zu müssen; tragen muss der Rest. Zwei Vorfassungen waren falsch und wurden vom Auftraggeber nacheinander korrigiert: die Anteilsregel von `PackSlowed` (C50) und die Mehrzahl des Radius (C51). Dass die Schwelle aus der Aktion kommt und nicht aus einer eigenen Option, hält beide Seiten derselben Frage zusammen und folgt einer geänderten Nutzereinstellung ohne zweite Stellschraube. Die Ersatzgarantie des Betäubungszweigs gilt unverändert: Ohne einen DoT mit eigenem Wert wird Sanctus gewirkt, gleich wie stark das Paket verlangsamt ist — deshalb werden aus fünfzehn Sekunden Verlangsamung keine fünfzehn Sekunden ohne Sanctus.
