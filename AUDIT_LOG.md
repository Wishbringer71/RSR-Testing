...
| C88 | A124, Code-Kommentar in `CustomRotation_Ability`, CLAUDE.md: `OnlyHealAsNonHealIfNoHealers` sei „der Regelfall und keine Ecke", und deshalb sei ein Beschwörer bei 1 Gesundheitspunkt an keinen Trank gekommen | Die Option steht ab Werk auf aus (`Configs.cs`). Belegt ist durch seine Beobachtung nur, dass **eine** der fünf geerbten Bedingungen bei ihm sperrte, nicht welche. Offen bleibt zudem, warum Upstream mit derselben Flagge bei ihm funktionierte | CLAUDE.md eingearbeitet; Code-Kommentar bei der nächsten Änderung an der Stelle; A133 |
| C89 | C79 und Code-Kommentar in `UseHpPotion`: Bei gleicher Heilung sei die niedrigere Gegenstands-Id die niedrigere Sorte | Die Heiltränke stehen in keiner Ressource des Repositorys; ob die Id-Reihenfolge der Sortenreihenfolge folgt, ist unbelegt. Aus dem Spiel ableitbar ist die Gegenstandsstufe | TODO, A133 |
